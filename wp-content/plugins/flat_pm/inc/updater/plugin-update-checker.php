...
class PluginUpdateChecker_3_1 {
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, 'Проверить есть ли новые обновления');
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = 'Этот плагин имеет актуальную версию.';
			} else if ( $status == 'update_available' ) {
				$message = 'Есть новая версия для плагина.';
			} else {
				$message = sprintf(__('Неизвестный статус обновления "%s"', 'plugin-update-checker'), htmlentities($status));
			}
			printf(
